...
9. 신규 / 텍스트박스 추가, 설비ID(A) 넣으면 어디 AP인지(B) 출력 (사용자가 미리 데이터 넣어야함)
...
9. 개선 / ServerMapping 접속버튼 클릭 시 , 접속 불가 상황이면 멈춤(로딩이길다)
10. 개선 / MGMT사용 후 닫았을 때, tick때문에 작업표시줄 계속 깜빡거림
11. 개선 / ServerMapping 시 Enter 후 일정 시간 후 버튼 비활성화 하기
